...
async function encontrarBitcoins(min, max) {
    ...
    while (key <= max && !shouldStop) {
        ...
        if (keysChecked % 1000000 === 0) {
            await writeFileAsync('Ultima_chave.txt', `Ultima chave tentada: ${pkey}`, true);
        }
...
async function writeFileAsync(fileName = filePath, data, overwrite = false) {
    return new Promise((resolve, reject) => {
        const options = { flag: overwrite ? 'w' : 'a' }; // 'w' para sobrescrever, 'a' para anexar
        fs.writeFile(fileName, data || buffer, options, (err) => {
            if (err) {
                reject(err);
            } else {
                buffer = Buffer.alloc(0); // Limpa o buffer após escrever no disco
